...
An examples of variable references with a `microprofile-config-alias.properties` file:

NOTE: The microprofile-config-alias.properties config source is only available in Payara Server since 5.202

[source,properties]
----
fish.payara.example.alias.system=${java.home}
fish.payara.example.alias.env=${ENV=PATH}
fish.payara.example.alias.password=${ALIAS=secret}
----